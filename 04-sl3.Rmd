...
1. Break up the sample, evenly, into $V$-folds (i.e., splits). Let's say we 
   have $n$ independent and identically distributed observations in the 
   sample.
2. For each of these (say $V=10$) folds:
    a. Remove that portion of the sample (kept out as validation sample) and 
       what's remaining will be used to fit learners (training sample).
    b. Fit each learner on the training sample. Let's say we have $k$ number 
       of learners.
    c. Predict the outcomes in the validation sample using each of the $k$ 
       trained learners. We will call these predictions "cross-validated"; since 
       they were obtained from the validation sample's covariate information, 
       which was never seen while fitting these models. Let's call 
       $n_{validation}$ the number of validation samples, then we would end up 
       with a $n_{validation} \by k$ matrix of "cross-validated" predictions. 
       Recall that $n_{validation}$ should only be 1/10 of the sample for 
       $V=10$-fold cross-validation.
 3. Bind together the rows of the 10 $n_{validation} \by k$ matrices of 
 ...
 4. Retain the observed outcome $Y$ for each of the $n$ observations, and use 
    them to measure the "loss" of each cross-validated prediction 
    (e.g., (Y − \hat{Y}^2$). Then, for each column, take the (potentially 
    weighted) mean across all of the $n$ losses, so we have a measure of 
    performance, which we call the "cross-validated empirical risk", for each 
    of the $k$ learners. 
 5. Choose the Super Learner (SL). The so-called "metalearner" function is 
    responsible for this, and the metalearner is just a function that assigns a 
    weight to each of the $k$ learner's. This weighted combination of $k$ 
    learners is the SL. 
  ...
  6. Fit the learners on the entire sample of $n$ observations, ignoring the 
     cross-validation folds/procedure, and use the weights that were obtained 
     in step 5 to get the SL fit. 
...
`r round(sl_fit_summary$mean_risk[length(sl_fit_summary$learner)], 4)` and that 
